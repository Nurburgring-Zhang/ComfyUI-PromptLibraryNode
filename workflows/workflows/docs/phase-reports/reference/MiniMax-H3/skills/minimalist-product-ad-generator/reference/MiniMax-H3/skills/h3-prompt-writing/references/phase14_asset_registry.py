# -*- coding: utf-8 -*-
"""
phase14_asset_registry.py — 资产层 (Asset Layer) Phase 14
==========================================================
...
ELEMENT_CONTEXT = {
    "current_node": "phase14_asset_registry",
    "function_of_asset": "身份合同 + 状态拆分 + 声音锁定 + 行为锁定 + 压力测试",
    "downstream_nodes": [
        "phase14_director_pro (环节 2, 调用 descriptor 进 H3 prompt)",
        "phase14_storyboard_pro (环节 3, 引用 ref 图 + 行为档案)",
        "phase14_cinedance (环节 4, 拼装 15 块刚性 prompt 骨架)",
        "phase14_acting (环节 5, 渲染 CHARACTER ACTING 区)",
    ],
...
def _get_asset(name):
    ...
    if name not in ASSET_REGISTRY:
        raise KeyError(
            "[phase14_asset_registry] 资产未注册: " + str(name) +
...
def get_state_descriptor(name, state):
    ...
    if state not in asset.states:
        ...
        return (
            "[phase14_asset_registry] 状态 '" + str(state) + "' 不存在于 " + name +
            " (可用: " + available + ")\n\n基础描述:\n" + asset.descriptor
        )
...
def render_asset_prompt(name, state=None):
    ...
    这是给 phase14_cinedance 节点用的最终输出。
    """
    asset = _get_asset(name)
...
class Phase14AssetRegistry:
    """
    ComfyUI 节点包装 — 让 phase14_asset_registry 模块能在 ComfyUI 工作流里直接调用。
    ...
    FUNCTION = "run"
    CATEGORY = "PromptLibrary/Phase14 资产层"
...
# ============================================================
# 9. 自检入口 — 直接 python phase14_asset_registry.py 跑一下
# ============================================================

if __name__ == "__main__":
    print("=" * 70)
    print("phase14_asset_registry — 自检")
    print("=" * 70)
    print()
    print("已注册资产:")
    print(list_assets())
