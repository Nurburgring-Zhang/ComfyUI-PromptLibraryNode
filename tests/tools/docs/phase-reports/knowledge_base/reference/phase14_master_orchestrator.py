...
def build_l1_l7_prompt(
    ...
    if not style_prefix:
        style_prefix = STYLE_PREFIX if _HAS_PHASE14_DOCS else "Photoreal. 8K IMAX. No 3D render. No game engine."
...
【15 块刚性骨架】
{chr(10).join(f'  {i+1:2d}. {b["name"]:<25s} — {b["purpose"]}' for i, b in enumerate(FIFTEEN_BLOCKS)) if _HAS_PHASE14_DOCS else '  (Phase 14 docs unavailable)'}
